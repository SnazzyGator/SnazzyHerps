...
public class BeardedDragonEntity extends TamableAnimal implements IAnimatable {
    ...
    public BeardedDragonEntity(EntityType<? extends TamableAnimal> entityType, Level level) {
        super(entityType, level);
        this.setCanPickUpLoot(true);
    }

    private static final EntityDataAccessor<Integer> VARIANT =
            SynchedEntityData.defineId(BeardedDragonEntity.class, EntityDataSerializers.INT);
    ...
    public AgeableMob getBreedOffspring(ServerLevel serverLevel, AgeableMob mob) {
        BeardedDragonEntity baby = ModEntityTypes.BEARDEDDRAGON.get().create(serverLevel);
        return baby;
    }
    ...
    private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
        if (!(event.getLimbSwingAmount() > -0.15F && event.getLimbSwingAmount() < 0.08F)) {
            if (isSprinting()) {
                event.getController().setAnimation(new AnimationBuilder().addAnimation("bdragon.run", true));
            } else {
                event.getController().setAnimation(new AnimationBuilder().addAnimation("bdragon.walk", true));
            }
        } else {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("bdragon.idle", true));
        }
    ...
    class HerpSearchForItemsGoal extends Goal {
        ...
        public boolean canUse() {
            if (!BeardedDragonEntity.this.getItemBySlot(EquipmentSlot.MAINHAND).isEmpty()) {
                return false;
            } else if (BeardedDragonEntity.this.getTarget() == null && BeardedDragonEntity.this.getLastHurtByMob() == null) {
                if (BeardedDragonEntity.this.getRandom().nextInt(reducedTickDelay(10)) != 0) {
                    return false;
                } else {
                    List<ItemEntity> list = BeardedDragonEntity.this.level.getEntitiesOfClass(ItemEntity.class, BeardedDragonEntity.this.getBoundingBox().inflate(8.0D, 8.0D, 8.0D), BeardedDragonEntity.ALLOWED_ITEMS);
                    return !list.isEmpty() && BeardedDragonEntity.this.getItemBySlot(EquipmentSlot.MAINHAND).isEmpty();
                }
            } else {
                return false;
            }
        ...
        public void tick() {
            List<ItemEntity> list = BeardedDragonEntity.this.level.getEntitiesOfClass(ItemEntity.class, BeardedDragonEntity.this.getBoundingBox().inflate(8.0D, 8.0D, 8.0D), BeardedDragonEntity.ALLOWED_ITEMS);
            ItemStack itemstack = BeardedDragonEntity.this.getItemBySlot(EquipmentSlot.MAINHAND);
            if (itemstack.isEmpty() && !list.isEmpty()) {
                BeardedDragonEntity.this.getNavigation().moveTo(list.get(0), (double)1.2F);
            }
