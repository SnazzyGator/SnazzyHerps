...
public class DayGeckoSmallEntity extends TamableAnimal implements IAnimatable {
    ...
    public DayGeckoSmallEntity(EntityType<? extends TamableAnimal> entityType, Level level) {
        super(entityType, level);
        this.setCanPickUpLoot(true);
    }

    private static final EntityDataAccessor<Integer> VARIANT =
            SynchedEntityData.defineId(DayGeckoSmallEntity.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<Byte> DATA_FLAGS_ID =
            SynchedEntityData.defineId(DayGeckoSmallEntity.class, EntityDataSerializers.BYTE);
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(5, new DayGeckoSmallEntity.HerpSearchForItemsGoal());
    ...
    public AgeableMob getBreedOffspring(ServerLevel serverLevel, AgeableMob mob) {
        DayGeckoSmallEntity baby = ModEntityTypes.DAYGECKOSMALL.get().create(serverLevel);
        return baby;
    }
    ...
    class HerpSearchForItemsGoal extends Goal {
        ...
        public boolean canUse() {
            if (!DayGeckoSmallEntity.this.getItemBySlot(EquipmentSlot.MAINHAND).isEmpty()) {
                return false;
            } else if (DayGeckoSmallEntity.this.getTarget() == null && DayGeckoSmallEntity.this.getLastHurtByMob() == null) {
                if (DayGeckoSmallEntity.this.getRandom().nextInt(reducedTickDelay(10)) != 0) {
                    return false;
                } else {
                    List<ItemEntity> list = DayGeckoSmallEntity.this.level.getEntitiesOfClass(ItemEntity.class, DayGeckoSmallEntity.this.getBoundingBox().inflate(8.0D, 8.0D, 8.0D), DayGeckoSmallEntity.ALLOWED_ITEMS);
                    return !list.isEmpty() && DayGeckoSmallEntity.this.getItemBySlot(EquipmentSlot.MAINHAND).isEmpty();
                }
            } else {
                return false;
            }
        ...
        public void tick() {
            List<ItemEntity> list = DayGeckoSmallEntity.this.level.getEntitiesOfClass(ItemEntity.class, DayGeckoSmallEntity.this.getBoundingBox().inflate(8.0D, 8.0D, 8.0D), DayGeckoSmallEntity.ALLOWED_ITEMS);
            ItemStack itemstack = DayGeckoSmallEntity.this.getItemBySlot(EquipmentSlot.MAINHAND);
            if (itemstack.isEmpty() && !list.isEmpty()) {
                DayGeckoSmallEntity.this.getNavigation().moveTo(list.get(0), (double)1.2F);
            }
