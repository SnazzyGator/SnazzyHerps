...
public class HouseGeckoEntity extends TamableAnimal implements IAnimatable {
    ...
    public HouseGeckoEntity(EntityType<? extends TamableAnimal> entityType, Level level) {
        super(entityType, level);
        this.setCanPickUpLoot(true);
    }

    private static final EntityDataAccessor<Integer> VARIANT =
            SynchedEntityData.defineId(HouseGeckoEntity.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<Byte> DATA_FLAGS_ID =
            SynchedEntityData.defineId(HouseGeckoEntity.class, EntityDataSerializers.BYTE);
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(5, new HouseGeckoEntity.HerpSearchForItemsGoal());
    ...
    public AgeableMob getBreedOffspring(ServerLevel serverLevel, AgeableMob mob) {
        HouseGeckoEntity baby = ModEntityTypes.HOUSEGECKO.get().create(serverLevel);
        return baby;
    }
    ...
    class HerpSearchForItemsGoal extends Goal {
        ...
        public boolean canUse() {
            if (!HouseGeckoEntity.this.getItemBySlot(EquipmentSlot.MAINHAND).isEmpty()) {
                return false;
            } else if (HouseGeckoEntity.this.getTarget() == null && HouseGeckoEntity.this.getLastHurtByMob() == null) {
                if (HouseGeckoEntity.this.getRandom().nextInt(reducedTickDelay(10)) != 0) {
                    return false;
                } else {
                    List<ItemEntity> list = HouseGeckoEntity.this.level.getEntitiesOfClass(ItemEntity.class, HouseGeckoEntity.this.getBoundingBox().inflate(8.0D, 8.0D, 8.0D), HouseGeckoEntity.ALLOWED_ITEMS);
                    return !list.isEmpty() && HouseGeckoEntity.this.getItemBySlot(EquipmentSlot.MAINHAND).isEmpty();
                }
            } else {
                return false;
            }
        ...
        public void tick() {
            List<ItemEntity> list = HouseGeckoEntity.this.level.getEntitiesOfClass(ItemEntity.class, HouseGeckoEntity.this.getBoundingBox().inflate(8.0D, 8.0D, 8.0D), HouseGeckoEntity.ALLOWED_ITEMS);
            ItemStack itemstack = HouseGeckoEntity.this.getItemBySlot(EquipmentSlot.MAINHAND);
            if (itemstack.isEmpty() && !list.isEmpty()) {
                HouseGeckoEntity.this.getNavigation().moveTo(list.get(0), (double)1.2F);
            }
